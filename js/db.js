...
(function (local) {
    ...
    var Server = function Server(db, name, version, noServerMethods) {
        ...
        var err = void 0;
        this.addStore = function(storeName) {
            console.log(storeName);
            _this2["$"][storeName] = {};
            var keys = Object.keys(_this2);
            keys.filter(function (key) {
                return ![].concat(serverEvents, ['close', 'addEventListener', 'removeEventListener']).includes(key) && !_this2[key].getIndexedDB && key !== storeName;
            }).map(function (key) {
                //console.log(key);
        ...
        [].some.call(db.objectStoreNames, this.addStore);
        return err;
    };
        ...
        Object.keys(schema).some(function (tableName) {
            ...
            if (db.objectStoreNames.contains(tableName)) {
                store = request.transaction.objectStore(tableName); // Shouldn't throw
                    ...
                    try {
                        //console.log(tableName, table.key);
